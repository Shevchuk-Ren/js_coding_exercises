import { findSmallNums, findNamesBeginningWith, findVerbs, getIntegers, getCities, getSquareRoots, findSentencesContaining, getLongestSides } from '../challenges/exercise004';
...
describe('findNamesBeginningWith', () => {
  test('returns an array of names beginning with the specified character', () => {
    ...
    expect(findNamesBeginningWith(names, 'S')).toEqual(['Sally', 'Susan', 'Sam']);
...
describe('findVerbs', () => {
  test("returns an array of words that are considered verbs (because they begin with 'to ')", () => {
    const words = ['to eat', 'fajita', 'mouse', 'to sneak', 'to squeak', 'cheesemonger'];
  ...
  test("does not mistake words that include 'to' elsewhere", () => {
    const words = ['to eat', 'tower', 'monitor', 'to sneak', 'to squeak', 'cheesemonger'];
    expect(findVerbs(words)).toEqual(['to eat', 'to sneak', 'to squeak']);
  });
});
...
describe('findSentencesContaining', () => {
  ...
  test('returns only the sentences containing the specified string', () => {
    expect(findSentencesContaining(sentencesAboutPackageJson, 'license')).toEqual(['You should specify a license for your package so that people know how they are permitted to use it']);

    expect(findSentencesContaining(sentencesAboutPackageJson, 'binary')).toEqual([]);
  });

  test('it should not be case sensitive', () => {
    expect(findSentencesContaining(sentencesAboutPackageJson, 'dependencies')).toEqual([
      'Dependencies are specified in a simple object that maps a package name to a version range',
      "It's best to map these additional items such as a test framework, which is not needed for running your project, in a devDependencies object",
    ]);
  });
